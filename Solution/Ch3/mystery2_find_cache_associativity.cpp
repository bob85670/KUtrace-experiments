...
Pair* MakeInboundLongList(uint8* ptr, int bytesize, int bytestride, bool makelinear) {
  int mixedup[256];
  mixedup[0] = 0;
  uint8 x = POLYINIT8;
  for (int i = 1; i < 256; ++i) {
  ...
  for (int i = 1; i < element_count; ++i) {
    int nextelement = makelinear ? i : (i & ~0xff) | mixedup[i & 0xff];
    if (nextelement >= element_count) nextelement = nextelement % element_count; // Keep within A elements
...
void FindCacheAssociativity(uint8* ptr, int kMaxArraySize, int linesize, int totalsize) {
  fprintf(stdout, "Finding associativity for cache size %d bytes, line size %d bytes\n", totalsize, linesize);

  bool makelinear = false;
  const int max_ways = 16;
  const int access_count = 256;

  // A is the number of elements in the linked list. Each element maps to a unique cache line in the same cache set
  for (int A = 1; A <= max_ways; ++A) {
    int bytesize = A * totalsize;
    if (bytesize > kMaxArraySize) {
      fprintf(stderr, "Error: bytesize %d exceeds kMaxArraySize %d for A=%d\n", bytesize, kMaxArraySize, A);
      continue;
    }
    // the linked list will have A elements spaced totalsize bytes apart, and all elements map to the same cache set set[0]
    Pair* pairptr = MakeInboundLongList(ptr, bytesize, totalsize, makelinear);
    TrashTheCaches(ptr, kMaxArraySize);

    // First pass: Load A lines into cache
    const Pair* loadptr = pairptr;
    for (int i = 0; i < A; ++i) {
      if (!loadptr) break;
      loadptr = loadptr->next;
    }

    // Second pass: Time 256 accesses, cycling through A lines
    int64 startcy = GetCycles();
    loadptr = pairptr; // Start from head
    for (int i = 0; i < access_count; ++i) {
      if (!loadptr) loadptr = pairptr; // Cycle through A elements
      loadptr = loadptr->next;
    }
    int64 stopcy = GetCycles();
    int64 cyclesperload = (stopcy - startcy) / access_count;

    if (gNeverZero == 0) { fprintf(stdout, "pairptr->data = %lld\n", pairptr->data); }
    fprintf(stdout, "A=%2d: %lld cy/ld\n", A, cyclesperload);
  }
}
